...
export default function Updates() {
    ...
    const safeUrl = (url) => url.startsWith('http://') ? url.replace('http://', 'https://') : url;
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                {articles.map(article => (
                    <a key={article.id} href={article.url} target="_blank" rel="noopener noreferrer"
                        className="hover:scale-105 sm:mb-0 mb-1 flex flex-col sm:flex-row bg-white dark:bg-[#212528] rounded-xl shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden">
